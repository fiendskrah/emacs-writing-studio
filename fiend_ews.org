:PROPERTIES:
:ID:       6F49F28A-86A3-4045-8106-79F356EA6E5D
:END:
+title: Fiend-EWS source doc
#+property: header-args :tangle init.el


* Source Documentation
This file is sourced from the appendix of Peter Prevos Emacs Writing Studio configuration. (=github.com/pprevos/emacs-writing-studio=)

I am starting with the EWS base config and adding in necessities from my config as they arise. The goal is that I will be able to interface with the emacs writing studio work flow to develop papers without losing the functionality that I have developed that /has/ worked for me.
...
**** /fiend/
This is where things I called 'Misc' went in my config. 

I like to see the line numbers, exclusive of certain modes
...
*I like to start in fullscreen*

#+BEGIN_SRC emacs-lisp
    (add-hook 'window-setup-hook 'toggle-frame-fullscreen t)
#+end_src

*When in doubt, push `esc` to hit da bricks*
...
*All the fonts*
  Need to run the all-the-fonts-install-fonts function to actually grab the fonts at first startup
  #+BEGIN_SRC emacs-lisp
...
#+end_src
  ...
  ;; This makes parenthesis, brackets, etc., colored and easier to read in programming languages.  
...
*python interpreter*
for my denote library pkb. Evaluate code using C-c, C-c
#+BEGIN_SRC emacs-lisp
...
The next section hooks the Variable Pitch mode to any buffer in text mode. This means that written prose is displayed in variable pitch, while metadata, code and other items are in fixed pitch. A hook is a construction in Emacs that associates modes with each other. In this case, variable pitch text is enabled for all text mode buffers.
...
*** Spellchecking
<s>Writing without automated spell-checking would be very hard, even for the most experienced authors. The Flyspell package requires the hunspell software and the relevant dictionary to be available. You should change the standard dictionary to your local variety with the ~ews-hunspell-dictionaries~  variable. EWS uses this particular variable because the dictionaries are set in two places. Section [[#sec-spelling]] explains how to use this package.</s>

I have replaced flyspell with a package called spell-fu - flyspell was causing issues with loading emacs
...
*** Org capture
:PROPERTIES:
:ID:       233EA93E-1AB3-4040-BEE1-4BCF1A614350
:END:
       ...
       (org-capture-templates
        '(("f" "Fleeting note"
          ...
          ("p" "Permanent note" plain
            (file denote-last-path) ; play with this if we get to denote
           :no-save t
...
** GENERAL
 ...
 really need my spacebar leader key.
...
I don't get exactly what this does tbh
...
Hydra is used to create keyboard macros. Most powerful when paired
with which-key.
...
How could I not use elements of DOOM?


; doom-modeline makes the bottom bar prettier
